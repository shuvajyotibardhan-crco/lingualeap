...
export default function Discovery({ level, phrases, onBack }) {
  return (
    <div className="min-h-screen bg-brand-yellow pb-10">
      ...
      <main className="max-w-lg mx-auto px-4 pt-6 flex flex-col gap-4">
        <p className="text-center text-sm text-gray-600 mb-2">
          Tap 🔊 on any phrase to hear it spoken.
        </p>
        {phrases.map(phrase => (
        ...
        <button
          onClick={onBack}
          className="mt-4 bg-brand-orange text-white font-bold rounded-2xl py-3 min-h-[44px] hover:opacity-90 transition-opacity"
        >
          Done exploring
        </button>
      </main>
